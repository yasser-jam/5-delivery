<template>
    <div class="flex items-center justify-between bg-white py-2 px-4 rounded-lg">
        
        <div class="flex items-center gap-6">
            <div class="flex items-center gap-2">
                <v-avatar icon="mdi-moped" size="small" class="text-white" color="primary" />
        
                <div class="text-secondary font-medium">Yasser Jamalaldeen</div>
            </div>
    
            <v-icon color="secondary">mdi-arrow-right-bold-outline</v-icon>
    
            <div class="flex items-center gap-2">
                <v-avatar icon="mdi-package" size="small" class="text-white" color="success" />
        
                <div class="text-secondary font-medium">Order - 1</div>
            </div>
        </div>
        

        <v-chip color="blue">In Progress</v-chip>
    </div>
</template>